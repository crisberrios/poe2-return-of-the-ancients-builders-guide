...
| - | --- | --- | --- | --- | --- | --- |
| 1 | Rolling Slam: first slam 90→75%, second slam 180→150% at base; attack time +1.0s (was +1.5s — each slam resolves 0.5s faster) | Skill Changes | **+ S** | **+ S** | **+ S** | Per community testing (see `community-notes.md`): the 0.5s faster cadence more than offsets the −17% damage. Effective DPS ≈ flat, with clearly better QoL. Reclassified from − L. |
...
| --- | --- | --- | --- |
| **Titan** | +0.0L · +1.0M · +0.7S = **+1.46** | +0.0L · −0.3M · −0.4S = **−0.34** | **+1.12** (net buff) |
| **Warbringer** | +1.0L · +1.5M · +0.8S = **+2.37** | +0.0L · −2.0M · −0.6S = **−1.16** | **+1.21** (net buff) |
| **Smith of Kitava** | +1.0L · +0.9M · +0.9S = **+1.88** | +0.0L · −0.6M · −0.5S = **−0.73** | **+1.15** (net buff) |

> Caveat: this rubric still weights the +Level to Melee Skills item nerf heavily — that change broadly hurts all three two-hand-mace builds. Rolling Slam, originally counted as the big skill-side nerf, has been reclassified to a small QoL **buff** (see `community-notes.md`); the "winners" picture is strengthened, driven by the **Ancestral Bond rework** (huge for totem playstyles), warcry/bleed uniques, and now the snappier Rolling Slam.
...
### Warbringer
- **Bleed Slam Warbringer (Rolling Slam → Boneshatter).** Used warcries for clear, instant leech for survivability. **0.5 impact: Voracious Notable removed instant leech (#10) and Boneshatter qual (#3) are the real hits; Rolling Slam (#1) is now a small QoL buff (faster cadence, ≈ flat DPS — see `community-notes.md`).** Net: defensive-layer loss rather than DPS loss; warcry CDR (#15) and bleed-fire conversion (#21) open new variants.
